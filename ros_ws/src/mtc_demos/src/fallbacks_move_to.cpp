...
int main(int argc, char** argv)
{
  ...
  // std::unordered_map<std::string, std::string> pilz_map = { { "pilz_industrial_motion_planner", "PTP" } };
  auto pilz_planner = std::make_shared<solvers::PipelinePlanner>(node);
  ...
  auto ompl_planner = std::make_shared<solvers::PipelinePlanner>(node);
  ...
  RCLCPP_INFO(logger, "Target state set for 'panda_arm' group");
