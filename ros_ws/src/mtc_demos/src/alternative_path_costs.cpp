...
int main(int argc, char** argv)
{
  ...
  robot_state.setToDefaultValues(robot_state.getJointModelGroup("panda_arm"), "ready");
  std::cout << "Initialized robot state in planning scene to default values and 'ready' position" << std::endl;
  ...
  // Strategy 3: Minimize end-effector motion
  {
    ...
    connect->setCostTerm(std::make_unique<cost::LinkMotion>("panda_joint7"));  // Distance traveled by the link in
                                                                               // meters
    alternatives->add(std::move(connect));
  ...
  // Strategy 4: Minimize elbow motion
  {
    ...
    connect->setCostTerm(std::make_unique<cost::LinkMotion>("panda_link3s"));
    alternatives->add(std::move(connect));
  ...
  // solving a particular part of the robot's movement. The planner can now choose the best strategy based on the
  // current situation.
  ...
  auto goal_joint_group = robot_state.getJointModelGroup("panda_arm");
  goal_scene->getCurrentStateNonConst().setToDefaultValues(goal_joint_group, "extended");
  std::cout << "Created goal scene with 'transport' position" << std::endl;
